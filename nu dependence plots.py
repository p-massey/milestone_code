...
test_nu_steps = np.logspace(2, 4, 50).astype(int)

total_L_nu_steps = [total_L(i) for i in test_nu_steps]

total_L_r_steps = [total_L_r(i) for i in test_nu_steps]

normalised_L = total_L_nu_steps/total_L_nu_steps[-1]

normalised_L_r = total_L_r_steps/total_L_r_steps[-1]

plt.figure(dpi=600)
plt.plot(np.log10(test_nu_steps), normalised_L, label='varied frequency bins')
plt.plot(np.log10(test_nu_steps), normalised_L_r, label='varied radial bins')
plt.xlabel(r'$log_{10}$[Bin number]')
