...
def nuLnu(acc):
    ...
    def T(rad):
        numerator = 3 * g_big_cgs * acc * mass * m_sol_cgs
...
def total_luminosity(ac):
    
    r_g = g_big_cgs * (mass*m_sol_cgs) / c_cgs**2
    
    def T(rad):
        numerator = 3 * g_big_cgs * ac * mass * m_sol_cgs
        denominator = 8 * np.pi * (rad)**3 * r_g**3 * sigma_cgs
        bracket = 1 - np.sqrt(r_in/(rad))
        return ((numerator/denominator)*bracket)**0.25
...
accretion_grid = np.linspace(np.log10(1e15), np.log10(m_dot(mass)), 9)
...
accretion_rate_smy = [10**i / m_sol_cgs * yr for i in accretion_grid]
print(accretion_rate_smy)
labels = [r'$1.6\times10^{-12} M_\odot yr^{-1}$', r'$1.9\times10^{-11} M_\odot yr^{-1}$', r'$2.3\times10^{-10} M_\odot yr^{-1}$', r'$2.7\times10^{-9} M_\odot yr^{-1}$',
          r'$3.2\times10^{-8} M_\odot yr^{-1}$', r'$3.9\times10^{-7} M_\odot yr^{-1}$', r'$4.6\times10^{-6} M_\odot yr^{-1}$', r'$5.6\times10^{-5} M_\odot yr^{-1}$',
          r'$6.7\times10^{-4} M_\odot yr^{-1}$']
print(labels)

plt.figure(dpi=1000)
for i in range (0,len(accretion_grid)):
    plt.scatter(log_nu_midpoints, nuLnu(10**accretion_grid[i]), s=1, label= labels[i])
